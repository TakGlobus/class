        ...
        if i > j:
          if k <= i//2:
            if array[i,j] > 0:
                ...
                ##loop = [ dp[ii,jj,k-1] for ii in range(0,j-1,1) for jj in range(0,j-1,1)]+[0]
                #loop = [ dp[j-1,jj,k-1] for ii in range(0,j-1,1) for jj in range(0,j-1,1)]+[0]
                #if len(loop) > 0:
                #    dp[i,j,k] = max(loop) + array[i,j]
              #elif k >= 2 :
                ##loop = [ dp[j-1,jj,k-1] for jj in range(0,j-1,1)]
                #loop = [ dp[ii,jj,k-1] for ii in range(0,j-1,1) for jj in range(0,j-1,1)]
                #if len(loop) > 0:
                #  if not max(loop) == 0:
                #    dp[i,j,k] = max(loop) + array[i,j]
                ...
                #tmp2 = max([dp[j-1,jj,k] for jj in range(0,j-1,1)]+[0]) 
                tmp2 = max([dp[j-1,jj,k] for jj in range(0,j,1)]+[0]) 
                #++ case: non-adding case 2
                #tmp3 = max([dp[i,jj,k] for jj in range(0,j-1,1)]+[0]) 
                tmp3 = max([dp[i,jj,k] for jj in range(0,j,1)]+[0]) 

                #if i == 6 and j == 4 and k == 2:
                #  print(array[i,j],tmp1,loop, tmp2,tmp3); exit(0)
              ...
              #dp[i,j,k] = max([dp[i, jj, k] for jj in range(0,j-1,1)]+[0])
