...
namespace TestFramework
{
   ...
   public:
      ...
      void RemoveLeft(size_t count);
      void RemoveRight(size_t count);
   ...
   template<class T>
   void AddColumn(TableAdapter<T>& tableAdapter, const char* name, int T::*field_pointer, int defaultValue = -1)
   {
   ...
   template<class T>
   void AddColumn(RecordAdapter<T>& recordAdapter, const char* name, int T::*field_pointer, int defaultValue = -1)
   {
      auto newAdapter = CreateSharedInterface(new FieldAdapter<T, int>(field_pointer, defaultValue));
      recordAdapter.AddNamedColumn(name, newAdapter);
   }
   ...
   inline
   void StringSegment::RemoveLeft(size_t count)
   {
      iBegin = std::min(iEnd, iBegin + count);
   }

   inline
   void StringSegment::RemoveRight(size_t count)
   {
